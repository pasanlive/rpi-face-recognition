...
def load_security_zone(camera_index: int) -> np.ndarray:
    """Load the security zone polygon for a given camera index.
    Returns an (N,2) int32 numpy array. If no entry exists, creates a default one.
    """
    table = _ensure_table()
    try:
        df = table.to_pandas()
        row = df[df["camera_index"] == camera_index]
        if not row.empty:
...
def save_security_zone(camera_index: int, polygon: List[List[int]]):
    ...
    try:
        table.delete(f"camera_index = {camera_index}")
    except Exception as e:
        logger.debug(f"No previous security zone to delete for camera {camera_index}: {e}")
    flat = _flatten_polygon(polygon)
    record = {"camera_index": camera_index, "polygon": flat}
    table.add([record])
    logger.info(f"Saved security zone for camera {camera_index}")
